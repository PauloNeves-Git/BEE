...
import pandas as pd
import torch
from utilities.classification_utils import InputExample, convert_examples_to_features, LazyClassificationDataset
...
class ClassificationModel:
        ...
        **kwargs,
        ...
        if isinstance(train_df, str):
            ...
            if "text" in train_df.columns and "labels" in train_df.columns:
                if "equivalents" in train_df.columns:
                    train_examples = [
                        InputExample(i, text, equivalents, None, label)
                        for i, (text, equivalents, label) in enumerate(zip(train_df["text"].astype(str), train_df["equivalents"].values, train_df["labels"]))
                    ]
                else:
                    train_examples = [ InputExample(i, text, None, None, label)
                                       for i, (text, label) in enumerate(zip(train_df["text"].astype(str), train_df["labels"]))]
            else:
                warnings.warn(
                    "Dataframe headers not specified. Falling back to using column 0 as text and column 1 as labels."
                )
                print("No text and labels columns on the df")

                train_examples = [
                    InputExample(i, text, None, None, label)
        ...
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=self.args["train_batch_size"]) #, num_workers=12

        os.makedirs(output_dir, exist_ok=True)

        global_step, tr_loss = self.train(train_dataloader, output_dir, train_df, multi_label=multi_label,
                                          show_running_loss=show_running_loss, eval_df=eval_df, verbose=verbose, **kwargs)
    ...
    def train(
        ...
        **kwargs,
		...
		# Only evaluate when single GPU otherwise metrics may not average well
            ...
            if args["save_model_every_epoch"] and args["save_epochs"] is None:
                self._save_model(output_dir_current, optimizer, scheduler, model=model)
            if args["save_model_every_epoch"] and args["save_epochs"] is not None:
                if epoch_number % args["save_epochs"] == 0:
                    self._save_model(output_dir_current, optimizer, scheduler, model=model)
    ...
    def evaluate(self, eval_df, output_dir, multi_label=False, prefix="", verbose=True, silent=False, **kwargs):
        ...
        if isinstance(eval_df, str):
            ...
            if "text" in eval_df.columns and "labels" in eval_df.columns:
                if "equivalents" in eval_df.columns:
                    eval_examples = [
                        InputExample(i, text, equivalents, None, label)
                        for i, (text, equivalents, label) in enumerate(zip(eval_df["text"].astype(str), eval_df["equivalents"].values, eval_df["labels"]))
                    ]
                else:
                    print("Using SMILES only to train the model.")
    ...
    def load_and_cache_examples(self, examples, evaluate=False, no_cache=False, multi_label=False, verbose=True, silent=False ):
        ...
        if os.path.exists(cached_features_file) and (
            ...
            print("Features were loaded for evaluation! ---------------------------------")
        ...
        all_equivalent_ids = torch.tensor([f.equivalent_ids for f in features], dtype=torch.long)
        ...
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_equivalent_ids, all_label_ids)
    ...
    def predict(self, to_predict, equivalents = None, multi_label=False):
        """
        ...
        if multi_label:
            if type(equivalents) != type(None):
            	eval_examples = [
                	InputExample(i, text, equivalents, None, [0 for i in range(self.num_labels)]) for (i, text), equivalents in zip(enumerate(to_predict),equivalents)]
            else:
                eval_examples = [ InputExample(i, text, None, None, [0 for i in range(self.num_labels)]) for i, text in enumerate(to_predict) ]
            ...
            if isinstance(to_predict[0], list):
                ...
                if type(equivalents) != type(None):
                    eval_examples = [InputExample(i, text, equivalents, None, 0) for (i, text), equivalents in zip(enumerate(to_predict), equivalents)]
                else:
                    eval_examples = [InputExample(i, text, None, None, 0) for i, text in enumerate(to_predict)]
        if args["sliding_window"]:
        ...
        if self.config.output_hidden_states:
            ...
            return preds, model_outputs, inputs
    ...
    def _get_inputs_dict(self, batch):
        if isinstance(batch[0], dict):
            ...
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "token_type_ids": batch[2], "equivalents_ids": batch[3], "labels": batch[-1]}
