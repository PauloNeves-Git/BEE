...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, equivalents = None, text_b=None, label=None):
        """
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, equivalent_ids, label_id):
...
def convert_example_to_feature(
    ...
    (example, max_seq_length, tokenizer, output_mode, cls_token, sep_token, cls_token_segment_id,
        pad_token_segment_id, multi_label, stride, pad_token,
    ) = example_row

    tokens_a = tokenizer.tokenize(example.text_a)
    text_size_1 = len(tokens_a)
    tokens_b = None
    if example.text_b:
    ...
    #modified
    if example.equivalents != None:
        equivalent_ids = example.equivalents
        if equivalent_ids[-1] == 0:
            equivalent_ids = equivalent_ids[:-1]
        equivalent_size_1 = len(example.equivalents)
        equivalent_ids += [sequence_a_segment_id]
        equivalent_size_2 = len(equivalent_ids)

    tokens = [cls_token] + tokens
    segment_ids = [cls_token_segment_id] + segment_ids

    if example.equivalents != None:
        equivalent_ids = [cls_token_segment_id] + equivalent_ids
        text_size_3 = len(tokens)
        equivalent_size_3 = len(equivalent_ids)

    else:
        equivalent_ids = segment_ids
    ...
    equivalent_ids = equivalent_ids + ([pad_token_segment_id] * padding_length)

    assert len(input_ids) == max_seq_length
    assert len(input_mask) == max_seq_length
    assert len(segment_ids) == max_seq_length
    assert len(equivalent_ids) == max_seq_length
    ...
    return InputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids, equivalent_ids=equivalent_ids, label_id=example.label,)
